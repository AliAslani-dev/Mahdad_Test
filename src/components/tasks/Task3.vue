<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-8 font-sans">
    <div class="bg-white shadow-lg rounded-xl p-8 w-full max-w-lg">
      ...
      <transition name="fade">
        <div v-if="showLoading" class="flex justify-center py-6">
          <div class="relative w-10 h-10">
            <div
              class="absolute inset-0 rounded-full border-4 border-t-transparent border-blue-500 animate-spin"
            ></div>
            <div
              class="absolute inset-0 rounded-full border-4 border-transparent border-b-purple-500 animate-spin-slow"
            ></div>
          </div>
        </div>
      </transition>
      ...
      <p
        style="margin-top: 10px; padding: 5"
        class="text-xs text-gray-500 mt-3 opacity-0 animate-fade-in"
      >
        💡 برای آزمایش حالت خطا و عملکرد مکانیزم جلوگیری از درخواست‌های متوالی، مسیر درخواست را در
        فایل مربوطه به یک آدرس اشتباه تغییر دهید و سپس چند بار دکمه بالا را بزنید.
      </p>
    </div>
  </div>
</template>
